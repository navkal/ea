    ...
    if ( sYaxisPrefix != "" )
    {
        ...
        if ( ( iMouseX >= tXaxis.min ) && ( iMouseX <= tXaxis.max ) && ( iMouseY >= tYaxis.min ) && ( iMouseY <= tYaxis.max ) )
        {
            ...
            for ( var iDataSet = 0; iDataSet < aDataSet.length; ++iDataSet )
            {
                ...
                var costNum = Number( costVal );
                var bUseCost = ( costVal != "" ) && ! isNaN( costNum );

                // Update the legend text for the current series
                var equal = ( tSeries.label.trim() == "" ) ? "" : "=";
                var dollar = bUseCost ? "$" : "";
                var value = bUseCost ? ( nY * costNum ).toFixed( 2 ) : nY.toFixed( tSeries.yaxis.tickDecimals );
                $("#plotview .legendLabel").eq( iDataSet ).text( tSeries.label + equal + dollar + value );
                console.log( "====> y=" + nY.toFixed( tSeries.yaxis.tickDecimals ) + " costNum=" + costNum + " value=" + value );
            }
        }
    }
}
