...
function plotDraw( tEvent )
{
   debugAddLine( "==>plotDraw( " + tEvent.type + " )" );
    ...
    // Update plot status
    plotStatusUpdate( ( g_aData.length == 0 ) ? 0 : g_aData[0].data.length, ( aDataFull.length == 0 ) ? 0 : aDataFull[0].data.length );
...
function plotStatusUpdate( iShowing, iOf )
{
  ...
  $( "#timestampFrom" ).text( "foo" );
  $( "#timestampTo" ).text( "moo" );
}
