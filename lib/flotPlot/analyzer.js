...
function plotFilter( event )
{
  var target = $( event.target );
  if ( target.prop( "checked" ) )
  {
    target.parent().find( "span" ).addClass( "bg-info" );
  }
  else
  {
    target.parent().find( "span" ).removeClass( "bg-info" );
  }
...
function downSampleControlsEnable()
{
  ...
  var iDensity = $( "#downSampleDensity_current" ).val();
  $( "#downSampleDensity" ).val( iDensity == 0 ? 100 : iDensity );
  $( "#downSampleShow" ).val( $( "#downSampleShow_current" ).val() );
  $( "#downSampleHide" ).val( $( "#downSampleHide_current" ).val() );
}
...
function plotDownSample()
{
    ...
    if ( bValidOffset && bValidDensity && bValidShow && bValidHide )
    {
        ...
        if ( ( iDensity != g_iDownSampleDensity ) || ( iOffset != g_iDownSampleOffset ) || ( iShow != g_iDownSampleShow ) || ( iHide != g_iDownSampleHide ) )
        {
            ...
            {
              document.getElementById("downSampleDensity_current").value = g_iDownSampleDensity = iDensity;
              $( "#downSampleDensity_current" ).css( "display", "inline" );
            }
            else
            {
              document.getElementById("downSampleDensity_current").value = g_iDownSampleDensity = 0;
              $( "#downSampleDensity_current" ).css( "display", "none" );
            }
