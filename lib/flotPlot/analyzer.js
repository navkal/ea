...
function plotDraw( tEvent )
{
   debugAddLine( "==>plotDraw( " + tEvent.type + " )" );
    ...
    // Show appropriate down-sample mode radio buttons
    downSampleModeShow();
...
// Show/hide down-sample modes
function downSampleModeShow()
{
  var range = $( "#downSampleRange" );

  // If no zoom range, force deselection of inapplicable mode
  if ( range.prop( "checked" ) && zoomRangeIsNull() )
  {
  ...
  // Show/hide mode
  range.parent().css( "display", zoomRangeIsNull() ? "none" : "inline" );
}
...
function downSampleControlsEnable()
{
  ...
  if ( $( "#downSampleRange" ).prop( "checked" ) )
  {
    $( "#downSampleShow" ).val( "NOT" );
    $( "#downSampleHide" ).val( "READY" );
    $( "#downSampleOffset" ).val( "YET!" );
  }
}
